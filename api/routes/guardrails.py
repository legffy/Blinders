...
async def update_guardrail(guardrail_id: UUID,body: GuardrailPatch, db: AsyncSession = Depends(get_db)) -> GuardrailOut:
    stmt = select(DomainGuardrail).where(DomainGuardrail.id == guardrail_id)
